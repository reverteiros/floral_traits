...
for (i in 1:nrow(BFNEWER)){
  ...
  for (j in 1:100){
    nsample<-sample(grisIT, size=BFNEWER[i,5],replace=T)
  ...
  hist(storage[[17]][,2],main='IT of Griseocollis visiting MOFI',xlab='IT length')
  
  abline(v=BFNEWER[17,3],col="red")
  #abline(v=3.36,col="blue")
simmean<-mean(storage[[17]][,2])
simmean
obsmean<-(BFNEWER[17,3])
obsmean
simsd<-sd(storage[[17]][,2])
simsd
