...
XYZ<-generaldata  %>% right_join(generaldata %>% complete(nesting(tongue_length.tongue, IT_improved), nesting(depth, width))) %>%  
  group_by(diff=abs(tongue_length.tongue-depth),  toodeep=tongue_length.tongue-depth<0, forbidden=toodeep*(as.numeric((IT_improved-width)>0)), bee, plant_gs) %>% 
...
m1<-glmer(y~scale(diff)+(1|bee), family="poisson", data=XYZ)
summary(m1)

#this is a negative binomial with pretty much all the predictors (many random)
m3<-glmer.nb(y~scale(diff)*forbidden+scale(plantab)+scale(beeab)+(1|bee)+(1|plant_gs),data=XYZ)
