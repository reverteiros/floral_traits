...
#######################################
# hist(longuetongues$tongue_length.tongue, xlab="Tongue length (mm)",main="",breaks=seq(8, 14, by = 1))
hist(longuetongues$depth, xlab="Flower depth (mm)",main="",breaks=seq(0, 31, by = 1))

# hist(shorttongues$tongue_length.tongue, xlab="Tongue length (mm)",main="",breaks=seq(0, 4, by = 1))
hist(shorttongues$depth, xlab="Flower depth (mm)",main="",breaks=seq(0, 31, by = 1))
#######################################3

# #### Plot quantile regresssion. Labels not working don't know why
# ggplot(shorttongues, aes(y=tongue_length.tongue, x=depth)) + 
#   geom_jitter(alpha=0.1, height=0.1) + 
#   theme_classic() +
#   abline(a=0, b =1)
# 
# ggplot(longuetongues, aes(y=tongue_length.tongue, x=depth)) + 
#   geom_jitter(alpha=0.1, height=0.1) + 
#   theme_classic() 

## how many bee species visit a plant considering flower depth
whatever<-subsetgeneraldata %>%
group_by(plant_gs) %>%
summarize(beespecies=n_distinct(bee),depth=mean(depth))

ggplot(whatever, aes(y=beespecies, x=depth)) + 
  geom_jitter() + 
  theme_classic()

proportion<-subsetgeneraldata %>%
  group_by(plant_gs) %>%
  summarize(visits=n())

proportionlong<-longuetongues %>%
  group_by(plant_gs) %>%
  summarize(longbees=n())

proportionshort<-shorttongues %>%
  group_by(plant_gs) %>%
  summarize(shortbees=n())

proportion <- proportion %>% left_join(proportionlong, by=(c("plant_gs")))
proportion <- proportion %>% left_join(proportionshort, by=(c("plant_gs")))

proportion<-proportion %>% mutate(proportionshort=(shortbees/visits*100))
proportion<-proportion %>% mutate(proportionlong=(longbees/visits*100))

subsetgeneraldata <- subsetgeneraldata %>% left_join(proportion, by=(c("plant_gs")))

proportions <-subsetgeneraldata %>%
  group_by(plant_gs) %>%
  summarize(depth=mean(depth), short=mean(proportionshort.y.y),longs=mean(proportionlong.y.y))


ggplot(proportions, aes(x=depth)) + 
  geom_jitter(aes(y=short)) + 
  ylim(-1,100) +
  # geom_jitter(aes(y=longs,col="red")) + 
  theme_classic()
