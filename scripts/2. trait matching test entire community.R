...
# 
# datasd <- datasd %>%
#   mutate(largest = ifelse(sd_obs>quantile975,T,F))%>%
#   mutate(morethan25=ifelse(sd_obs>quantile25,T,F))%>%
#   mutate(group = case_when(largest & morethan25 ~ "1.Strongest variaiton", # both tests: group A
#                            !largest & morethan25 ~ "2.random variation", # one test: group B
#                            !largest & !morethan25 ~ "3.small variation" # neither test: group C
#   ))
# 
# 
...
#   theme_classic()
# 
# ggplot(data = datasd, aes(x=group, y=tongue)) + 
#   geom_violin()+
#   geom_point()+
#   theme_classic()+
#   theme(legend.position = "none") 
# 
# ggplot(data = datasd, aes(x=tongue)) + 
#   geom_density(fill="lightblue",alpha=0.6)+
#   theme_classic()+
#   facet_grid(group ~ .)+
#   theme(legend.position = "none") 
...
# ggplot(observed, aes(y=sd_obs, x=abundance)) + 
